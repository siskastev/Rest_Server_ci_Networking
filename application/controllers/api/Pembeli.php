...
//$this->response(array("status"=>"success"));
	...
	function user_get()
	{
		$get_pembeli = $this->db->query("SELECT p.id_pembeli, p.nama, p.alamat,
			p.telpn, p.photo_id FROM pembeli as p")->result();
		$this->response(array("status"=>"success","result" => $get_pembeli));
	}
	function user_post() {$action = $this->post('action');
	$data_pembeli = array(
...
//cek validasi
	if (empty($data_pembeli['id_pembeli'])){
		$this->response(array('status' => "failed", "message"=>"Id Pembeli harus
			diisi"));
	}else if (empty($data_pembeli['nama'])){
		$this->response(array('status' => "failed", "message"=>"nama harus
			diisi"));
	}else if (empty($data_pembeli['alamat'])){
		$this->response(array('status' => "failed", "message"=>"alamat harus
			diisi"));
	}else if (empty($data_pembeli['telpn'])){
		$this->response(array('status' => "failed", "message"=>"telpn harus
			diisi"));
	}
	else{
		$get_pembeli_baseid = $this->db->query("SELECT * FROM pembeli as p WHERE
			p.id_pembeli='".$data_pembeli['id_pembeli']."'")->result();
		if(empty($get_pembeli_baseid)){
			...
			$this->response(array('status' => "failed", "message"=>"Id_pembeli
				sudah ada"));
		}
	}
}
...
//jika photo di update eksekusi query
					$update= $this->db->query("Update pembeli Set nama
						='".$data_pembeli['nama']."', alamat ='".$data_pembeli['alamat']."' , telpn
						='".$data_pembeli['telpn']."', photo_id ='".$data_pembeli['photo_id']."' Where id_pembeli
						='".$data_pembeli['id_pembeli']."'");
					$data_pembeli['photo_id'] = base_url()."upload/".$user_img;
				}else{
//jika photo di kosong atau tidak di update eksekusi query
					$update= $this->db->query("Update pembeli Set nama
						='".$data_pembeli['nama']."', alamat ='".$data_pembeli['alamat']."' , telpn
						='".$data_pembeli['telpn']."' Where id_pembeli ='".$data_pembeli['id_pembeli']."'");
					$getPhotoPath =$this->db->query("SELECT photo_id
						FROM pembeli Where id_pembeli='".$data_pembeli['id_pembeli']."'")->result();
	...
	function deletePembeli($data_pembeli){
		if (empty($data_pembeli['id_pembeli'])){
			...
			$getPhotoPath =$this->db->query("SELECT photo_id FROM pembeli Where
				id_pembeli='".$data_pembeli['id_pembeli']."'")->result();
			if(!empty($getPhotoPath)){
				foreach ($getPhotoPath as $row)
				{
					$path = str_replace("application/", "",
						APPPATH).$row->photo_id;
				}
//delete image
				unlink($path);
				$this->db->query("Delete From pembeli Where
					id_pembeli='".$data_pembeli['id_pembeli']."'");
				$this->response(array('status'=>'success',"message"=>"Data id =
					".$data_pembeli['id_pembeli']." berhasil di delete "));
			} else{
